...
TEST_F(BackendCassandraTest, Basic)
{
    ...
    boost::asio::spawn(
        ctx_,
        [this, &done, &work](boost::asio::yield_context yield) {
            ...
            }
            {
                ...
                EXPECT_TRUE(obj);
                EXPECT_STREQ(
                    reinterpret_cast<char const*>(obj->data()), static_cast<char const*>(accountBlobOld.data())
                );
                obj = backend_->fetchLedgerObject(key256, lgrInfoOld.seq - 1, yield);
                EXPECT_FALSE(obj);
            }
            ...
            auto checkLedger = [&](auto lgrInfo, auto txns, auto objs, auto accountTx) {
                ...
                for (auto [account, data] : accountTx) {
                    ...
                    for (size_t i = 0; i < retData.size(); ++i) {
                        ...
                        EXPECT_STREQ(
                            reinterpret_cast<char const*>(txn.data()), static_cast<char const*>(expTxn.data())
                        );
                        EXPECT_STREQ(
                            reinterpret_cast<char const*>(meta.data()), static_cast<char const*>(expMeta.data())
                        );
                    }
                }
