...
TEST_F(SubscriptionManagerAsyncTest, MultipleThreadCtx)
{
    ...
    constexpr static auto kJSON_MANIFEST = R"({"manifest":"test"})";
    constexpr static auto kJSON_VALIDATION = R"({"validation":"test"})";
...
{
    constexpr static auto kREPORT_RETURN =
        R"({
            "ledger":0,
            "transactions":2,
...
{
    constexpr static auto kDUMMY_MANIFEST = R"({"manifest":"test"})";
    EXPECT_CALL(*sessionPtr_, onDisconnect);
...
{
    constexpr static auto kDUMMY = R"({"validation":"test"})";
    EXPECT_CALL(*sessionPtr_, onDisconnect);
...
TEST_F(SubscriptionManagerTest, LedgerTest)
{
    ...
    constexpr static auto kLEDGER_RESPONSE =
        R"({
    ...
    fee2.reserve = 10;
    constexpr static auto kLEDGER_PUB =
        R"({
            "type":"ledgerClosed",
            "ledger_index":31,
            "ledger_hash":"4BC50C9B0D8515D3EAAE1E74B29A95804346C491EE1A95BF25E4AAB854A6A652",
...
TEST_F(SubscriptionManagerTest, ProposedTransactionTest)
{
    ...
    auto const metaObj = createMetaDataForBookChange(kCURRENCY, kACCOUN_T1, 22, 3, 1, 1, 3);
    trans1.metadata = metaObj.getSerializer().peekData();
