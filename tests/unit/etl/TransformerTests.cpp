...
constexpr static auto kRAW_HEADER =
...
struct ETLTransformerTest : util::prometheus::WithPrometheus, MockBackendTest {
    ...
    LedgerLoaderType ledgerLoader;
    LedgerPublisherType ledgerPublisher;
    AmendmentBlockHandlerType amendmentBlockHandler;
    SystemState state;

    std::unique_ptr<TransformerType> transformer;
...
TEST_F(ETLTransformerTest, StopsOnWriteConflict)
{
    ...
    EXPECT_CALL(dataPipe, popNext).Times(0);
    EXPECT_CALL(ledgerPublisher, publish(_)).Times(0);

    transformer = std::make_unique<TransformerType>(
        dataPipe, backend_, ledgerLoader, ledgerPublisher, amendmentBlockHandler, 0, state
    );

    transformer->waitTillFinished();  // explicitly joins the thread
}
...
TEST_F(ETLTransformerTest, StopsOnEmptyFetchResponse)
{
    ...
    ON_CALL(dataPipe, popNext).WillByDefault([this, &response](auto) -> std::optional<FakeFetchResponse> {
        if (state.isStopping)
            return std::nullopt;
    ...
    EXPECT_CALL(dataPipe, popNext).Times(AtLeast(1));
    EXPECT_CALL(*backend_, startWrites).Times(AtLeast(1));
    EXPECT_CALL(*backend_, writeLedger(_, _)).Times(AtLeast(1));
    EXPECT_CALL(ledgerLoader, insertTransactions).Times(AtLeast(1));
    ...
    EXPECT_CALL(ledgerPublisher, publish(_)).Times(AtLeast(1));
...
TEST_F(ETLTransformerTest, DoesNotPublishIfCanNotBuildNextLedger)
{
    ...
    ON_CALL(dataPipe, popNext).WillByDefault(Return(response));
