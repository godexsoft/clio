...
constexpr static auto kACCOUNT = "rM2AGCCCRb373FRuD8wHyUwUsh2dV4BW5Q";
constexpr static auto kNFTID = "0008013AE1CD8B79A8BCB52335CD40DE97401B2D60A828720000099B00000000";
constexpr static auto kNFTI_D2 = "05FB0EB4B899F056FA095537C5817163801F544BAFCEA39C995D76DB4D16F9DA";
constexpr static auto kOFFE_R1 = "23F1A95D7AAB7108D5CE7EEAF504B2894B8C674E6D68499076441C4837282BF8";
constexpr static auto kTX = "13F1A95D7AAB7108D5CE7EEAF504B2894B8C674E6D68499076441C4837282BF8";

struct NFTHelpersTests : public NoLoggerFixture {};

TEST_F(NFTHelpersTests, ConvertDataFromNFTCancelOfferTx)
{
    auto const tx = createCancelNftOffersTxWithMetadata(kACCOUNT, 1, 2, std::vector<std::string>{kNFTI_D2, kNFTID});
...
TEST_F(NFTHelpersTests, ConvertDataFromNFTCancelOfferTxContainingDuplicateNFT)
{
    auto const tx = createCancelNftOffersTxWithMetadata(
        kACCOUNT, 1, 2, std::vector<std::string>{kNFTI_D2, kNFTID, kNFTI_D2, kNFTID}
    );
    ripple::TxMeta const txMeta(ripple::uint256(kTX), 1, tx.metadata);
    auto const [nftTxs, nftDatas] =
        etl::getNFTDataFromTx(txMeta, ripple::STTx(ripple::SerialIter{tx.transaction.data(), tx.transaction.size()}));
...
TEST_F(NFTHelpersTests, UniqueNFTDatas)
{
    ...
    auto const generateNFTsData = [](char const* nftID, std::uint32_t txIndex) {
        auto const tx = createCreateNftOfferTxWithMetadata(kACCOUNT, 1, 50, nftID, 123, kOFFE_R1);
    ...
    nftDatas.push_back(generateNFTsData(kNFTI_D2, 4));
    nftDatas.push_back(generateNFTsData(kNFTI_D2, 1));
    nftDatas.push_back(generateNFTsData(kNFTI_D2, 5));
    ...
    EXPECT_EQ(uniqueNFTDatas[0].tokenID, ripple::uint256(kNFTI_D2));
