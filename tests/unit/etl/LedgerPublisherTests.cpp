...
TEST_F(ETLLedgerPublisherTest, PublishLedgerHeaderIsWritingFalseAndCacheDisabled)
{
    ...
    EXPECT_CALL(*backend_, fetchLedgerDiff(Seq, _)).Times(0);
    ...
    EXPECT_EQ(backend_->fetchLedgerRange().value().minSequence, Seq);
    EXPECT_EQ(backend_->fetchLedgerRange().value().maxSequence, Seq);
}
...
TEST_F(ETLLedgerPublisherTest, PublishLedgerHeaderIsWritingFalseAndCacheEnabled)
{
    ...
    EXPECT_CALL(*backend_, fetchLedgerDiff(Seq, _)).Times(1);
...
TEST_F(ETLLedgerPublisherTest, PublishLedgerHeaderInRange)
{
    ...
    impl::LedgerPublisher publisher(ctx_, backend_, mockCache, mockSubscriptionManagerPtr, dummyState);
    backend_->setRange(Seq - 1, Seq);

    publisher.publish(dummyLedgerHeader);

    // mock fetch fee
    EXPECT_CALL(*backend_, doFetchLedgerObject(ripple::keylet::fees().key, Seq, _))
...
TEST_F(ETLLedgerPublisherTest, PublishLedgerHeaderCloseTimeGreaterThanNow)
{
    ...
    // mock fetch transactions
    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger(Seq, _))
...
TEST_F(ETLLedgerPublisherTest, PublishLedgerSeqMaxAttampt)
{
    ...
    EXPECT_CALL(*backend_, hardFetchLedgerRange).Times(MaxAttempt).WillRepeatedly(Return(range));
...
TEST_F(ETLLedgerPublisherTest, PublishLedgerSeqStopIsFalse)
{
    ...
    auto const dummyLedgerHeader = createLedgerHeader(LedgerHash, Seq, Age);
    EXPECT_CALL(*backend_, fetchLedgerBySequence(Seq, _)).WillOnce(Return(dummyLedgerHeader));
    EXPECT_CALL(mockCache, isDisabled).WillOnce(Return(false));
    EXPECT_CALL(*backend_, fetchLedgerDiff(Seq, _)).WillOnce(Return(std::vector<LedgerObject>{}));
    EXPECT_CALL(mockCache, updateImp);
