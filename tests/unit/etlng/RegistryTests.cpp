...
{
    auto [sttx, meta] = util::CreateNftTxAndMeta();
...
TEST(RegistryTest, FilteringOfTxWorksCorrectlyForInitialTransaction)
{
...
TEST(RegistryTest, FilteringOfTxWorksCorrectlyForTransaction)
{
    ...
    EXPECT_CALL(extOffer, onTransaction(testing::_, testing::_)).Times(1);  // 1 create offer
...
TEST(RegistryTest, OnLedgerDataForBatch)
{
    ...
    EXPECT_CALL(ext, onLedgerData(testing::_)).Times(1);  // 1 batch (dispatch call)
