...
constexpr static auto kNFTI_D2 = "00081388319F12E15BCA13E1B933BF4C99C8E1BBC36BD4910A85D52F00000022";
...
{
    constexpr static auto kCURRENT_OUTPUT = R"({
...
TEST_F(RPCNFTInfoHandlerTest, BurnedNFT)
{
    ...
    // fetch nft return something
    auto const nft =
        std::make_optional<NFT>(createNft(kNFTID, kACCOUNT, ledgerHeader.seq, ripple::Blob{'u', 'r', 'i'}, true));
    ON_CALL(*backend_, fetchNFT).WillByDefault(Return(nft));
    EXPECT_CALL(*backend_, fetchNFT(ripple::uint256{kNFTID}, 30, _)).Times(1);
...
TEST_F(RPCNFTInfoHandlerTest, NotBurnedNFTWithoutURI)
{
    ...
    // fetch nft return something
    auto const nft = std::make_optional<NFT>(createNft(kNFTID, kACCOUNT, ledgerHeader.seq, ripple::Blob{}));
    ON_CALL(*backend_, fetchNFT).WillByDefault(Return(nft));
    EXPECT_CALL(*backend_, fetchNFT(ripple::uint256{kNFTID}, 30, _)).Times(1);
...
TEST_F(RPCNFTInfoHandlerTest, NFTWithExtraFieldsSet)
{
    ...
    // fetch nft return something
    auto const nft = std::make_optional<NFT>(createNft(kNFTI_D2, kACCOUNT, ledgerHeader.seq));
    ON_CALL(*backend_, fetchNFT).WillByDefault(Return(nft));
    EXPECT_CALL(*backend_, fetchNFT(ripple::uint256{kNFTI_D2}, 30, _)).Times(1);
