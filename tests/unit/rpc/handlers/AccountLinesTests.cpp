...
constexpr static auto kACCOUN_T3 = "rB9BMzh27F3Q6a5FtGPDayQoCCEdiRdqcK";
constexpr static auto kINDE_X1 = "E6DBAFC99223B42257915A63DFC6B0C032D4070F9A574B255AD97466726FC321";
...
TEST_F(RPCAccountLinesHandlerTest, DefaultParameterTest)
{
    ...
    ripple::STObject const ownerDir =
        createOwnerDirLedgerObject({ripple::uint256{kINDE_X1}, ripple::uint256{kINDE_X2}}, kINDE_X1);
...
TEST_F(RPCAccountLinesHandlerTest, UseLimit)
{
    ...
    auto repetitions = 50;
    while ((repetitions--) != 0) {
        indexes.emplace_back(kINDE_X1);
        auto const line =
            createRippleStateLedgerObject("USD", kACCOUN_T2, 10, kACCOUNT, 100, kACCOUN_T2, 200, kTXNID, 123);
        bbs.push_back(line.getSerializer().peekData());
    }
    ripple::STObject ownerDir = createOwnerDirLedgerObject(indexes, kINDE_X1);
...
TEST_F(RPCAccountLinesHandlerTest, UseDestination)
{
    ...
    while ((repetitions--) != 0) {
        indexes.emplace_back(kINDE_X1);
        auto const line =
            createRippleStateLedgerObject("USD", kACCOUN_T3, 10, kACCOUNT, 100, kACCOUN_T3, 200, kTXNID, 123);
        bbs.push_back(line.getSerializer().peekData());
    }

    ripple::STObject const ownerDir = createOwnerDirLedgerObject(indexes, kINDE_X1);
    ON_CALL(*backend_, doFetchLedgerObject(owneDirKk, testing::_, testing::_))
        .WillByDefault(Return(ownerDir.getSerializer().peekData()));
    EXPECT_CALL(*backend_, doFetchLedgerObject).Times(2);
...
TEST_F(RPCAccountLinesHandlerTest, EmptyChannel)
{
    ...
    // return owner index
    ripple::STObject const ownerDir = createOwnerDirLedgerObject({}, kINDE_X1);
...
{
    constexpr static auto kCORRECT_OUTPUT = R"({
        "account": "rf1BiGeXwwQoi8Z2ueFYTEXSwuJYfV2Jpn",
        "ledger_hash": "4BC50C9B0D8515D3EAAE1E74B29A95804346C491EE1A95BF25E4AAB854A6A652",
        "ledger_index": 30,
        "validated": true,
    ...
    std::vector<Blob> bbs;
    auto line1 = createRippleStateLedgerObject("USD", kACCOUN_T2, 10, kACCOUNT, 100, kACCOUN_T2, 200, kTXNID, 0);
    line1.setFlag(ripple::lsfHighAuth);
    line1.setFlag(ripple::lsfHighNoRipple);
    line1.setFlag(ripple::lsfHighFreeze);
    bbs.push_back(line1.getSerializer().peekData());

    auto line2 = createRippleStateLedgerObject("USD", kACCOUN_T2, 20, kACCOUNT, 200, kACCOUN_T2, 400, kTXNID, 0);
...
TEST_F(RPCAccountLinesHandlerTest, MarkerOutput)
{
    ...
    constexpr static auto kNEXT_PAGE = 99;
    constexpr static auto kLIMIT = 15;
    ...
    std::vector<Blob> bbs;
    auto line = createRippleStateLedgerObject("USD", kACCOUN_T2, 10, kACCOUNT, 100, kACCOUN_T2, 200, kTXNID, 0);
    ...
    ON_CALL(*backend_, doFetchLedgerObject(ownerDirKk, testing::_, testing::_))
        .WillByDefault(Return(ownerDir.getSerializer().peekData()));
    ripple::STObject ownerDir2 = createOwnerDirLedgerObject(indexes, kINDE_X1);
    ...
    runSpawn([&, this](auto yield) {
        ...
        EXPECT_EQ(
            ...
            fmt::format("{},{}", kINDE_X1, kNEXT_PAGE)
        );
...
TEST_F(RPCAccountLinesHandlerTest, MarkerInput)
{
    ...
    std::vector<Blob> bbs;
    auto const line = createRippleStateLedgerObject("USD", kACCOUN_T2, 10, kACCOUNT, 100, kACCOUN_T2, 200, kTXNID, 0);
