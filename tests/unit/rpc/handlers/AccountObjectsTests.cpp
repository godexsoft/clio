...
constexpr static auto kISSUER = "rsA2LpzuawewSBQXkiju3YQTMzW13pAAdW";
...
TEST_F(RPCAccountObjectsHandlerTest, LedgerNonExistViaIntSequence)
{
    ...
    EXPECT_CALL(*backend_, fetchLedgerBySequence(kMAXSEQ, _)).WillOnce(Return(std::optional<ripple::LedgerHeader>{}));
...
{
    static auto constexpr kEXPECTED_OUT = R"({
    ...
    EXPECT_CALL(*backend_, doFetchLedgerObject(accountKk, kMAXSEQ, _)).WillOnce(Return(Blob{'f', 'a', 'k', 'e'}));

    auto const ownerDir = createOwnerDirLedgerObject({ripple::uint256{kINDE_X1}}, kINDE_X1);
...
TEST_F(RPCAccountObjectsHandlerTest, Limit)
{
    ...
    static auto constexpr kLIMIT = 10;
    auto count = kLIMIT * 2;
    // put 20 items in owner dir, but only return 10
    auto const ownerDir = createOwnerDirLedgerObject(std::vector(count, ripple::uint256{kINDE_X1}), kINDE_X1);
    auto const ownerDirKk = ripple::keylet::ownerDir(account).key;
    EXPECT_CALL(*backend_, doFetchLedgerObject(ownerDirKk, 30, _))
        .WillOnce(Return(ownerDir.getSerializer().peekData()));
    ...
    while (count-- != 0) {
        auto const line1 =
            createRippleStateLedgerObject("USD", kISSUER, 100, kACCOUNT, 10, kACCOUN_T2, 20, kTXNID, 123, 0);
        bbs.push_back(line1.getSerializer().peekData());
    }
    EXPECT_CALL(*backend_, doFetchLedgerObjects).WillOnce(Return(bbs));
    ...
    runSpawn([&](auto yield) {
        ...
        EXPECT_EQ(output.result->as_object().at("marker").as_string(), fmt::format("{},{}", kINDE_X1, 0));
    });
}
...
TEST_F(RPCAccountObjectsHandlerTest, MultipleDirNoNFT)
{
    ...
    static auto constexpr kCOUNT = 10;
    static auto constexpr kNEXTPAGE = 1;
    auto cc = kCOUNT;
    auto ownerDir = createOwnerDirLedgerObject(std::vector(cc, ripple::uint256{kINDE_X1}), kINDE_X1);
    // set next page
    ...
    runSpawn([&](auto yield) {
        ...
        EXPECT_EQ(output.result->as_object().at("marker").as_string(), fmt::format("{},{}", kINDE_X1, kNEXTPAGE));
    });
}
...
TEST_F(RPCAccountObjectsHandlerTest, TypeFilter)
{
    ...
    auto const ownerDir = createOwnerDirLedgerObject({ripple::uint256{kINDE_X1}, ripple::uint256{kINDE_X1}}, kINDE_X1);
    auto const ownerDirKk = ripple::keylet::ownerDir(account).key;
    EXPECT_CALL(*backend_, doFetchLedgerObject(ownerDirKk, 30, _))
        .WillOnce(Return(ownerDir.getSerializer().peekData()));
...
TEST_F(RPCAccountObjectsHandlerTest, TypeFilterAmmType)
{
    ...
    auto const ammObject = createAmmObject(kACCOUNT, "XRP", toBase58(ripple::xrpAccount()), "JPY", kACCOUN_T2);
...
TEST_F(RPCAccountObjectsHandlerTest, DeletionBlockersOnlyFilter)
{
    ...
    auto const line = createRippleStateLedgerObject("USD", kISSUER, 100, kACCOUNT, 10, kACCOUN_T2, 20, kTXNID, 123, 0);
    auto const channel = createPaymentChannelLedgerObject(kACCOUNT, kACCOUN_T2, 100, 10, 32, kTXNID, 28);
    auto const offer = createOfferLedgerObject(
        kACCOUNT,
        10,
        20,
...
TEST_F(RPCAccountObjectsHandlerTest, NFTMixOtherObjects)
{
    ...
    auto const nftPage2KK =
        ripple::keylet::nftpage(ripple::keylet::nftpage_min(account), ripple::uint256{kINDE_X1}).key;
    auto const nftpage1 =
        createNftTokenPage(std::vector{std::make_pair<std::string, std::string>(kTOKENID, "www.ok.com")}, nftPage2KK);
...
TEST_F(RPCAccountObjectsHandlerTest, NFTReachLimitReturnMarker)
{
    ...
    for (auto i = 0; i < 10; i++) {
        ...
        auto const nftpage =
            createNftTokenPage(std::vector{std::make_pair<std::string, std::string>(kTOKENID, "www.ok.com")}, previous);
...
TEST_F(RPCAccountObjectsHandlerTest, NFTMarker)
{
    ...
    auto const ownerDir = createOwnerDirLedgerObject(
        {ripple::uint256{kINDE_X1}, ripple::uint256{kINDE_X1}, ripple::uint256{kINDE_X1}}, kINDE_X1
    );
    auto const ownerDirKk = ripple::keylet::ownerDir(account).key;
    EXPECT_CALL(*backend_, doFetchLedgerObject(ownerDirKk, 30, _))
        .WillOnce(Return(ownerDir.getSerializer().peekData()));
...
TEST_F(RPCAccountObjectsHandlerTest, NFTMarkerNotExist)
{
    ...
    EXPECT_CALL(*backend_, doFetchLedgerObject(accountNftMax, kMAXSEQ, _)).WillOnce(Return(std::nullopt));
...
TEST_F(RPCAccountObjectsHandlerTest, NFTLimitAdjust)
{
    ...
    runSpawn([&](auto yield) {
        ...
        EXPECT_EQ(output.result.value().as_object().at("marker").as_string(), fmt::format("{},{}", kINDE_X1, 0));
    });
}
