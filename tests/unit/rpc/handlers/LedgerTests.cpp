...
constexpr static auto kINDE_X2 = "1B8590C01B0006EDFA9ED60296DD052DC5E90F99659B25014D08E1BC983515B1";
...
TEST_F(RPCLedgerHandlerTest, QueryViaLedgerHash)
{
    auto const ledgerHeader = createLedgerHeader(kLEDGERHASH, kRANGEMAX);
    EXPECT_CALL(*backend_, fetchLedgerByHash).Times(1);
    ON_CALL(*backend_, fetchLedgerByHash(ripple::uint256{kINDE_X1}, _)).WillByDefault(Return(ledgerHeader));

    runSpawn([&, this](auto yield) {
        auto const handler = AnyHandler{LedgerHandler{backend_}};
        auto const req = json::parse(fmt::format(R"({{"ledger_hash": "{}" }})", kINDE_X1));
        auto output = handler.process(req, Context{yield});
        ASSERT_TRUE(output);
        EXPECT_TRUE(output.result->as_object().contains("ledger"));
    });
...
TEST_F(RPCLedgerHandlerTest, TransactionsExpandBinary)
{
    ...
    t1.transaction = createPaymentTransactionObject(kACCOUNT, kACCOUN_T2, 100, 3, kRANGEMAX).getSerializer().peekData();
    t1.metadata = createPaymentTransactionMetaObject(kACCOUNT, kACCOUN_T2, 110, 30).getSerializer().peekData();
    t1.ledgerSequence = kRANGEMAX;

    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger).Times(1);
    ON_CALL(*backend_, fetchAllTransactionsInLedger(kRANGEMAX, _)).WillByDefault(Return(std::vector{t1, t1}));
...
TEST_F(RPCLedgerHandlerTest, TransactionsExpandBinaryV2)
{
    ...
    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger(kRANGEMAX, _)).WillOnce(Return(std::vector{t1, t1}));
...
TEST_F(RPCLedgerHandlerTest, TransactionsExpandNotBinary)
{
    ...
    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger).Times(1);
    ON_CALL(*backend_, fetchAllTransactionsInLedger(kRANGEMAX, _)).WillByDefault(Return(std::vector{t1}));
...
TEST_F(RPCLedgerHandlerTest, TransactionsExpandNotBinaryV2)
{
    ...
    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger(kRANGEMAX, _)).WillOnce(Return(std::vector{t1}));
...
TEST_F(RPCLedgerHandlerTest, TwoRequestInARowTransactionsExpandNotBinaryV2)
{
    ...
    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger(kRANGEMAX, _)).WillOnce(Return(std::vector{t1}));
    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger(kRANGEMAX - 1, _)).WillOnce(Return(std::vector{t1}));
...
TEST_F(RPCLedgerHandlerTest, TransactionsNotExpand)
{
    ...
    ON_CALL(*backend_, fetchAllTransactionHashesInLedger(kRANGEMAX, _))
        .WillByDefault(Return(std::vector{ripple::uint256{kINDE_X1}, ripple::uint256{kINDE_X2}}));
    ...
    runSpawn([&, this](auto yield) {
        ...
        EXPECT_EQ(
            ...
            json::parse(fmt::format(R"(["{}","{}"])", kINDE_X1, kINDE_X2))
        );
    });
}
...
TEST_F(RPCLedgerHandlerTest, DiffNotBinary)
{
    ...
    los.push_back(LedgerObject{ripple::uint256{kINDE_X2}, Blob{}});  // NOLINT(modernize-use-emplace)
    los.push_back(LedgerObject{
        ripple::uint256{kINDE_X1},
        createAccountRootObject(kACCOUNT, ripple::lsfGlobalFreeze, 1, 10, 2, kINDE_X1, 3).getSerializer().peekData()
    });
...
TEST_F(RPCLedgerHandlerTest, OwnerFundsTrueBinaryFalse)
{
    ...
    auto const accountObject =
        createAccountRootObject(kACCOUNT, 0, kRANGEMAX, 200 /*balance*/, 2 /*owner object*/, kINDE_X1, kRANGEMAX - 1, 0)
            .getSerializer()
            .peekData();
    ON_CALL(*backend_, doFetchLedgerObject(accountKk, kRANGEMAX, _)).WillByDefault(Return(accountObject));

    // fee object 2*2+3->7 ; balance 200 - 7 -> 193
    auto feeBlob = createLegacyFeeSettingBlob(1, 2 /*reserve inc*/, 3 /*reserve base*/, 4, 0);
    ON_CALL(*backend_, doFetchLedgerObject(ripple::keylet::fees().key, kRANGEMAX, _)).WillByDefault(Return(feeBlob));

    EXPECT_CALL(*backend_, doFetchLedgerObject).Times(2);

    TransactionAndMetadata tx;
    tx.metadata = createMetaDataForCreateOffer(kCURRENCY, kACCOUN_T2, 100, 300, 200).getSerializer().peekData();
    tx.transaction = createCreateOfferTransactionObject(kACCOUNT, 2, 100, kCURRENCY, kACCOUN_T2, 200, 300, true)
                         .getSerializer()
                         .peekData();
    tx.date = 123456;
    ...
    EXPECT_CALL(*backend_, fetchAllTransactionsInLedger).Times(1);
    ON_CALL(*backend_, fetchAllTransactionsInLedger(kRANGEMAX, _)).WillByDefault(Return(std::vector{tx}));
...
TEST_F(RPCLedgerHandlerTest, OwnerFundsNotEnoughForReserve)
{
    ...
    auto const accountKk = ripple::keylet::account(getAccountIdWithString(kACCOUNT)).key;
    auto const accountObject =
        createAccountRootObject(kACCOUNT, 0, kRANGEMAX, 6 /*balance*/, 2 /*owner object*/, kINDE_X1, kRANGEMAX - 1, 0)
            .getSerializer()
            .peekData();
    ON_CALL(*backend_, doFetchLedgerObject(accountKk, kRANGEMAX, _)).WillByDefault(Return(accountObject));
...
TEST_F(RPCLedgerHandlerTest, OwnerFundsNotXRP)
{
    ...
    auto const line = createRippleStateLedgerObject(
        kCURRENCY, kACCOUN_T2, 50 /*balance*/, kACCOUNT, 10, kACCOUN_T2, 20, kINDE_X1, 123
    );
    ...
    TransactionAndMetadata tx;
    tx.metadata = createMetaDataForCreateOffer(kCURRENCY, kACCOUN_T2, 100, 300, 200, true).getSerializer().peekData();
    tx.transaction = createCreateOfferTransactionObject(kACCOUNT, 2, 100, kCURRENCY, kACCOUN_T2, 200, 300)
                         .getSerializer()
                         .peekData();
    tx.date = 123456;
    tx.ledgerSequence = kRANGEMAX;
