...
constexpr static auto kACCOUN_T1 = "rsA2LpzuawewSBQXkiju3YQTMzW13pAAdW";
...
TEST_F(RPCAccountInfoHandlerTest, SignerListsInvalid)
{
    ...
    auto const accountRoot = createAccountRootObject(kACCOUNT, 0, 2, 200, 2, kINDE_X1, 2);
    ...
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kDISALLOW_INCOMING, _)).WillOnce(Return(false));
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kCLAWBACK, _)).WillOnce(Return(false));
...
TEST_F(RPCAccountInfoHandlerTest, SignerListsTrueV2)
{
    ...
    auto signersKey = ripple::keylet::signers(account).key;
    ON_CALL(*backend_, doFetchLedgerObject(signersKey, 30, _))
        .WillByDefault(Return(createSignerLists({{kACCOUN_T1, 1}, {kACCOUN_T2, 1}}).getSerializer().peekData()));
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kDISALLOW_INCOMING, _)).WillOnce(Return(false));
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kCLAWBACK, _)).WillOnce(Return(false));
    EXPECT_CALL(*backend_, doFetchLedgerObject).Times(2);
...
TEST_F(RPCAccountInfoHandlerTest, DisallowIncoming)
{
    ...
    ON_CALL(*backend_, doFetchLedgerObject(accountKk, 30, _))
        .WillByDefault(Return(accountRoot.getSerializer().peekData()));
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kDISALLOW_INCOMING, _)).WillOnce(Return(true));
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kCLAWBACK, _)).WillOnce(Return(false));
    EXPECT_CALL(*backend_, doFetchLedgerObject);
...
TEST_F(RPCAccountInfoHandlerTest, Clawback)
{
    ...
    ON_CALL(*backend_, doFetchLedgerObject(accountKk, 30, _))
        .WillByDefault(Return(accountRoot.getSerializer().peekData()));
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kDISALLOW_INCOMING, _)).WillOnce(Return(false));
    EXPECT_CALL(*mockAmendmentCenterPtr_, isEnabled(_, Amendments::kCLAWBACK, _)).WillOnce(Return(true));
    EXPECT_CALL(*backend_, doFetchLedgerObject);
