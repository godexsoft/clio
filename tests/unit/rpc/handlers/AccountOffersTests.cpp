...
TEST_F(RPCAccountOffersHandlerTest, DefaultParams)
{
    ...
    auto constexpr kLEDGER_SEQ = 30;

    auto const ledgerHeader = createLedgerHeader(kLEDGERHASH, kLEDGER_SEQ);
    EXPECT_CALL(*backend_, fetchLedgerBySequence).Times(1);
...
TEST_F(RPCAccountOffersHandlerTest, Limit)
{
    ...
    auto const ownerDir = createOwnerDirLedgerObject(std::vector{20, ripple::uint256{kINDE_X1}}, kINDE_X1);
    auto const ownerDirKk = ripple::keylet::ownerDir(getAccountIdWithString(kACCOUNT)).key;
    ON_CALL(*backend_, doFetchLedgerObject(ownerDirKk, kLEDGER_SEQ, _))
        .WillByDefault(Return(ownerDir.getSerializer().peekData()));
    EXPECT_CALL(*backend_, doFetchLedgerObject).Times(2);
    ...
    runSpawn([&](auto yield) {
        ...
        EXPECT_EQ(output.result->at("marker").as_string(), fmt::format("{},0", kINDE_X1));
    });
}
...
TEST_F(RPCAccountOffersHandlerTest, LimitLessThanMin)
{
    ...
    auto const ownerDir = createOwnerDirLedgerObject(
        std::vector{AccountOffersHandler::kLIMIT_MIN + 1, ripple::uint256{kINDE_X1}}, kINDE_X1
    );
    auto const ownerDirKk = ripple::keylet::ownerDir(getAccountIdWithString(kACCOUNT)).key;
    ON_CALL(*backend_, doFetchLedgerObject(ownerDirKk, kLEDGER_SEQ, _))
        .WillByDefault(Return(ownerDir.getSerializer().peekData()));
...
TEST_F(RPCAccountOffersHandlerTest, LimitMoreThanMax)
{
    ...
    auto const ownerDir = createOwnerDirLedgerObject(
        std::vector{AccountOffersHandler::kLIMIT_MAX + 1, ripple::uint256{kINDE_X1}}, kINDE_X1
    );
