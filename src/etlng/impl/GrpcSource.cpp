...
#include "etlng/impl/AsyncData.hpp"
...
GrpcSource::loadInitialLedger(uint32_t const sequence, uint32_t const numMarkers, etlng::LoaderInterface& loader)
{
    ...
    std::vector<etlng::impl::AsyncCallData> calls = etlng::impl::makeAsyncCallData(sequence, numMarkers);
    ...
    while (numFinished < calls.size() && cq.Next(&tag, &ok)) {
        ...
        auto ptr = static_cast<etlng::impl::AsyncCallData*>(tag);
        ...
        auto result = ptr->process(stub_, cq, loader, abort);
        if (result != etlng::impl::AsyncCallData::CallStatus::MORE) {
            ++numFinished;
