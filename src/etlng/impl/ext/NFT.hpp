...
public:
    ...
    // TODO: note how this code is similar to onInitialTransactions below. we could unify
    void
    ...
    {
        LOG(log_.trace()) << "got object with key = " << obj.key;
    ...
    {
        LOG(log_.trace()) << "got initial TXS cnt = " << data.size();

        std::vector<NFTsData> nfts;
        std::vector<NFTTransactionsData> nftTxs;

        for (auto const& tx : data) {
            auto const [txs, maybeNFT] = etl::getNFTDataFromTx(tx.meta, tx.sttx);
            nftTxs.insert(nftTxs.end(), txs.begin(), txs.end());
            if (maybeNFT)
                nfts.push_back(*maybeNFT);
