...
public:
    CoreExt(std::shared_ptr<BackendInterface> backend) : backend_(std::move(backend))
    {
    }
    ...
    insertTransactions(model::LedgerData const& data) const
    {
        for (auto const& txn : data.transactions) {
            ...
            backend_->writeTransaction(
                auto{txn.key},
                data.seq,
                data.header.closeTime.time_since_epoch().count(),  // TODO: this prevents from moving to onTransaction
                auto{txn.raw},
                auto{txn.metaRaw}
            );
        }
