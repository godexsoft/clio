...
)
{
    ...
    if (txn->isFieldPresent(ripple::sfAmount)) {
        ...
        static std::uint32_t constexpr kFIRST_LEDGER_WITH_DELIVERED_AMOUNT = 4594095;
        static std::uint32_t constexpr kDELIVERED_AMOUNT_LIVE_DATE = 446000000;
...
{
    static uint32_t constexpr kMAX_LEDGER_SEQ = 0x0FFF'FFFF;
    if (ledgerSeq > kMAX_LEDGER_SEQ)
        return {};

    static uint64_t constexpr kCTID_PREFIX = 0xC000'0000;
...
)
{
    ...
    static std::uint32_t constexpr kMIN_NODES = 2048;
...
parseRippleLibSeed(boost::json::value const& value)
{
    ...
    static std::size_t constexpr kSEED_SIZE = 18;
    static std::array<std::uint8_t, 2> constexpr kSEED_PREFIX = {0xE1, 0x4B};
