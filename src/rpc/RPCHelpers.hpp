...
decodeCTID(T const ctid) noexcept
{
    auto const getCTID64 = [](T const ctid) noexcept -> std::optional<uint64_t> {
        if constexpr (std::is_convertible_v<T, std::string>) {
            ...
            static std::size_t constexpr kCTID_STRING_LENGTH = 16;
    ...
    static uint64_t constexpr kCTID_PREFIX = 0xC000'0000'0000'0000ULL;
    static uint64_t constexpr kCTID_PREFIX_MASK = 0xF000'0000'0000'0000ULL;
...
logDuration(web::Context const& ctx, T const& dur)
{
    ...
    static util::Logger const kLOG{"RPC"};
    static std::int64_t constexpr kDURATION_ERROR_THRESHOLD_SECONDS = 10;
    ...
    if (seconds > kDURATION_ERROR_THRESHOLD_SECONDS) {
        LOG(kLOG.error()) << ctx.tag() << msg;
    } else if (seconds > 1) {
        LOG(kLOG.warn()) << ctx.tag() << msg;
    } else
        LOG(kLOG.info()) << ctx.tag() << msg;
}
