...
LedgerDataHandler::process(Input input, Context const& ctx) const
{
    ...
    if (input.diffMarker) {
        ...
        auto const limit =
            std::min(input.limit, input.binary ? LedgerDataHandler::kLIMITBINARY : LedgerDataHandler::kLIMITJSON);
...
tag_invoke(boost::json::value_to_tag<LedgerDataHandler::Input>, boost::json::value const& jv)
{
    ...
    if (jsonObject.contains(JS(binary))) {
        ...
        input.limit = input.binary ? LedgerDataHandler::kLIMITBINARY : LedgerDataHandler::kLIMITJSON;
    }
